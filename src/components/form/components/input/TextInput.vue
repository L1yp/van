<template>
  <template v-if="cMode === 'design'">
    <el-input v-model="val" disabled v-bind="props.inputProps" />
  </template>
  <template v-else-if="cMode === 'edit'">
    <el-input v-model="val" v-bind="props.inputProps"  />
  </template>
...
import { FormFieldMode, InputProps } from "@/components/form/types";
