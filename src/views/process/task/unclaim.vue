    ...
    <div class="data-table">
      ...
      >
        ...
        <el-table-column fixed="right" label="操作">
          <template #default="scope">
            <el-button size="small" plain style="vertical-align: middle" text @click.stop="claimTask(scope.row)">
              <SVGIcon style="width: 1em; height: 1em" name="Download"/><span style="margin-left: 6px">认领</span>
            </el-button>
...
import {listUnclaimTask} from "@/api/sys/process";
...
async function claimTask(row: ProcessTODOTaskView) {
  try {
    await ProcessApi.claimTask(row.task_id)
    await loadTableData()
    ElMessage.success('认领成功, 请前往我的待办处理该任务')
  } catch (e) {
    console.error(e)
...
async function loadTableData() {
  ...
  dictInfoTableData.value = await ProcessApi.listUnclaimTask()
